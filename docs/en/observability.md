...
- **Reproducible smokes**:
  - `bash scripts/otlp-collector-smoke.sh`: a real `otelcol-contrib` (file/debug exporters) plus cell-agent + user-runtime + a probe worker; asserts the collector received spans (with `cellhive.namespace`), a log record carrying the caller `trace_id`, and an `ns`-labelled binding counter on `/metrics`.
  - `bash scripts/openobserve-e2e.sh`: a real **OpenObserve** behind the repo's reference collector config, validating the backend hop (ran green: `OPENOBSERVE-E2E: PASS`): logs land in `default logs` (`cellhive_namespace=obs`, `trace_id` preserved) and a slow (>1s) request survives the reference `tail_sampling` keep-slow policy so the whole trace lands in `default traces` (spans carry `cellhive_namespace`/`cellhive_binding`).
  - **Backend field naming**: OpenObserve flattens attribute dots (`cellhive.namespace` -> query column `cellhive_namespace`); query traces with `POST /api/<org>/_search?type=traces`, and the span name is in `operation_name`.
