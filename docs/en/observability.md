...
- **Go spans** (cell-agent): `http.server` (per request, `http.route`/`http.status_code` + `cellhive.namespace/kind/name/scope`), `cell.durability_proof` (`Capture.Wait`), `peer.append` (each replica sent to a follower, with scope attributes). do-supervisor gate/restore also uses the same exporter.
