...
- dev: hot reload/Queue/`rules`/assets/`--strict-build` (Go+esbuild `internal/bundler` + `cellhive bundle build`) have been implemented; the old Miniflare pair aligned assets `_headers`/`_redirects`/`not_found_handling` + worker fallback with the production loader. **During the Miniflare 5 upgrade, its built-in router's `has_user_worker` cannot satisfy not-found handling and worker fallback together. ADR-114 now permits a same-instance, no-extra-listener, dev-only service-binding router inside `cli/`; the upgrade remains in progress until the full assets and hot-reload smoke passes.**
