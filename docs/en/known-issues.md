...
- **Version pin**: Miniflare-bundled workerd (4.20260714.0 → 1.20260714.1) is **overridden to platform pinned 1.20260615.1**; if override is impossible, drift is recorded.
...
- **pin rule (important correction)**: the Miniflare version must be aligned with the platform pinned workerd **from the same period**; **do not** override the workerd of a newer Miniflare (such as 4.20260714.0) to the older `1.20260615.1`—its internal control worker **hardcodes** `compatibilityDate` (4.20260714.0 = `2026-07-08`), and the older workerd (upper bound 2026-06-22) fails at startup. Current choice: **`miniflare@4.20260616.0` + `overrides.workerd=1.20260615.1`** (validated in practice). The previous spike that "only `require`/`dispatchFetch`" missed the control service that is only instantiated in dev-server mode.
...
- dev: hot reload/Queue/`rules`/assets/`--strict-build` (Go+esbuild `internal/bundler` + `cellhive bundle build`) have been implemented; **assets `_headers`/`_redirects`/`not_found_handling` + worker fallback have been aligned with the production loader (ADR-114, `make cli-test`)**.
...
| R2 `writeHttpMetadata` (ADR-174→ADR-175 fixed) | ✅ **Resolved**: `loader.js`/`buildFacetEnv` passes `CH_R2_BINDINGS`; `queue-wrapper`/`bindings-wrapper` wrap each R2 binding with a local Proxy (`facades.js wrapR2Metadata`); `writeHttpMetadata` mutates the caller’s `Headers` inside the tenant isolate, and content-type works correctly; only `get()` is proxied, all other methods pass through |
...
| WebSocket upgrade on DO stub (ADR-174→ADR-175 fixed) | ✅ **Resolved**: facade detects `Upgrade` → cell-agent `GET /v1/do/connect` (scoped) to get `{owner,ticket}` → owner do-runtime `/v1/do/connect` (ticket is only for this shard); the `wsecho` example WS echo passes. Alarm identity (`storage_id/storage_class`) and do-runtime address scheme were fixed in the same batch |
