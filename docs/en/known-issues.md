...
| C-02 | bundle/assets read path | ✅ Implemented: runtime requests a short-lived read-only presigned URL from cell-agent, then reads object storage directly; local-filesystem/unsupported-presign backends fall back to authenticated internal point reads (ADR-030) |
...
|---|---|---|
| Whether workerd actor SQLite is in WAL mode and can be opened read-only externally | ✅ Verified | Historical P0.7 evidence: `walscan` parsed real workerd actor `-wal` (workerd 2026-06-15) |
...
| **S3 authority condition semantics** | ❌ 2026-09-22 `make s3-test` with pinned MinIO: four `s3probe` writes and range passed, replication/restore passed, but `TestS3BucketIntegration` found stale `DeleteObject If-Match` returned success | Full authority contract fails for this image. Require target CAS and conditional delete on each deployed instance; the earlier four-write probe alone is not acceptance |
...
- **Authority bucket compatibility**: S3 `ConditionalDelete` relies on `DeleteObject If-Match`; ignoring it could erase a newer owner/lease, so `cell-agent` exits on any failed startup probe. `CELLHIVE_BUCKET` accepts empty (local), `s3://`, and native append-backed `oss://` / `cos://`; other schemes fail closed. Native providers use `AppendObject(position)` and tombstone frames for mutable authority CAS/delete, never a non-atomic `HEAD+PUT/DELETE` substitute. On 2026-09-22, isolated concurrent claims, CAS/conditional delete, and startup diagnose passed on the OSS Beijing bucket and COS Hong Kong `cell-1376795072` bucket; same-host dual-process `SIGKILL`/fresh-directory takeover against each real cloud bucket and high-concurrency exact-value checks also passed (ADR-189). The earlier COS Hong Kong `vwork-hk-1376795072` bucket returned 405 for AppendObject and remains unqualified. Ordinary COS/OSS S3 API paths failed the tested authority conditions (ADR-188). Other buckets need independent qualification; cross-host partition and long-running chaos remain untested.
...
**Implemented (real workerd e2e)**: `internal/workflow` (`__workflow__` cell: instances/steps/events, step memoization); facade `env.WF.create/createBatch/get/sendEvent`; cell-agent API (tenant create/create-batch/get/event, internal step/sleep/finish); user-runtime `/v1/workflows/run` + `cellhive-workflow.js` base + `step.do/sleep` (shim: rewrites `cloudflare:workers`, constructed by the platform, because workerd's `WorkflowEntrypoint` cannot be constructed outside the engine); sleep reuses the unified timer (`KindWorkflowSleep`); CLI `workflow create`; `wranglercompat` requires `class_name`.

**Partial boundaries (not done)**: cross-worker instances; `locationHint` is accepted but **ignored** (best-effort placement hint, no effect); step-history listing and progress callbacks.
**Implemented**: `pause`/`resume`/`terminate`/`restart`, `waitForEvent`, instance listing, idempotent `instance.delete()`, per-step `retries{limit,delay,backoff}`, `createBatch` (1..100 instances in one cell transaction), and the 1 MiB total create/createBatch request budget.
...
| Tenant console logs (ADR-185) | ⚠️ **Unsupported for now**: tested pinned stock `workerd 1.20260916.1` rejects a service designator in `tails: [{name:"tenant-tail"}]` for a dynamic `workerLoader` Worker: `TypeError: Incorrect type for array element 0: the provided value is not of type 'Fetcher'.` The old `log-tail.js`/`PlatformBridge.logSend` bridge is removed; tenant requests and DO facets still execute, but `console.*` does not enter the platform ring or OTLP. Do not restore this path with a tenant-env token, URL, or transport; rerun the real dynamic-tail spike after upgrading the pin. |
| workerd runtime baseline (ADR-186) | ✅ **First phase accepted**: pin `1.20260916.1`, esbuild `0.28.2`, host `fromEnvironment`, generated compatibility manifest, and two-layer final-WorkerCode 64 MiB / env 1016 KiB budgets are implemented. Isolated Docker acceptance proves source bundling, env isolation, KV, gated-DO bucket durability and restart continuity; `REQUIRE_ALL=1` reports **GATE: PASS 14/14**. Loader lifecycle, DO mid-flight fencing/restart generation, native Tail/OTLP, and compatible multi-module artifacts remain separate later phases; Tail has no tenant-env fallback. |
...
| WebSocket upgrade on DO stub (ADR-174→ADR-175 fixed) | ✅ **Resolved (no tenant transport needed since ADR-184)**: both workerLoader boundaries stay fetch-shaped: public loader → `CellHiveHost.fetch(request)`, then tenant facade → platform-side `DurableObjectNamespace.fetch(request)`. The latter stamps `DO_ID_HEADER`, performs the scoped cell-agent `{owner,ticket}` lookup, and connects to the owner do-runtime. `TestTenantDoWebSocketPassesThroughPublicLoader` proves an external 101 plus duplex frames; `TestTenantDoWebSocketCrossesRpc` proves tenant-side socket consumption; gated Docker acceptance proves public WS and persistent counts across restart. `CH_DO_CONNECT`/`CELLHIVE_CAP_WS` remain removed. |
