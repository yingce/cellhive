...
| workerd runtime baseline (ADR-186) | ◐ **Code implemented; final gate pending**: pin `1.20260916.1`, esbuild `0.28.2`, host `fromEnvironment`, generated compatibility manifest, and two-layer final-WorkerCode 64 MiB / env 1016 KiB budgets are implemented; real user-runtime/do-runtime workerd suites pass. Task 11's isolated Docker Compose E2E and `REQUIRE_ALL=1` gate remain pending, so the phase must not yet be called complete. Tail was reprobed on the new pin and remains unsupported, with no tenant-env fallback. |
