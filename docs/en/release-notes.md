...
### Security: Role Credentials Removed From the Tenant Env (ADR-074 boundary)
- `tenantEnv`/`buildFacetEnv` no longer copy `CELL_TOKEN` or `LOG_TOKEN` into the tenant-visible env object, so tenant code can no longer read internal/log-role credentials (a long-standing divergence from the ADR-074 rule "the internal token never enters the loaded worker").
...
### Tenant env carries no platform transport: DO/Workflow/Vectorize platform-side stubs (ADR-184; env conclusions superseded by ADR-185)
...
- DO WebSocket now rides the same platform-side stub: `DurableObjectNamespace.fetch(request)` (fetch-shaped RPC; the DO id travels on the request via `DO_ID_HEADER`). workerd keeps 101/WebSocket passthrough only for that shape, and the owner lookup + shard ticket stay inside the platform worker. **`CH_DO_CONNECT` and `CELLHIVE_CAP_WS` are gone** (so is the `cap-ws` network): the tenant env holds no credential and no tenant-visible transport. Arbitrary DO methods still use the `rpcObject` data path (ADR-162). Workflow step callbacks and the tenant log ring merge into the data-only `PlatformBridge` (`CH_PLATFORM`) stub, whose identity (ns/workflow/id/run, ns/worker) is bound in props; `WF_*`/`LOG_NS`/`LOG_WORKER` are no longer in env.
- **Two platform env keys + a reserved namespace**: binding-name lists (R2/DO) are injected via module-scope consts, the dead `CH_FACADE_SPEC` is gone, and workflow steps + log ingest merge into one `PlatformBridge` — the tenant env now holds **only `CH_PLATFORM`** (the bridge). `CH_*`/`CELL_*`/`__cellhive*` and the platform identity keys are reserved; user vars/secrets/binding names there are rejected at deploy/write time (`reserved_env_name`).
