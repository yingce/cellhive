...
- DO WebSocket — the one case that cannot cross workerLoader RPC — goes through a **cluster-only service binding `CH_DO_CONNECT`** (`CELLHIVE_CAP_WS` controls its allow list; unset falls back to public+private for back-compat, production should set the cluster subnet). The owner lookup and shard ticket come from the platform-side `connectInfo()`, so **no credential remains in the tenant env**. Workflow step callbacks and the tenant log ring move to data-only `WorkflowSteps`/`LogSink` stubs whose identity (ns/workflow/id/run, ns/worker) is bound in props; `WF_*`/`LOG_NS`/`LOG_WORKER` are no longer in env.
- **Two platform env keys + a reserved namespace**: binding-name lists (R2/DO) are injected via module-scope consts, the dead `CH_FACADE_SPEC` is gone, and workflow steps + log ingest merge into one `PlatformBridge` — the tenant env now holds only `CH_PLATFORM` (the bridge) and, for DO workers, `CH_DO_CONNECT` (the cluster-only transport for the DO WebSocket upgrade). `CH_*`/`CELL_*`/`__cellhive*` and the platform identity keys are reserved; user vars/secrets/binding names there are rejected at deploy/write time (`reserved_env_name`).
