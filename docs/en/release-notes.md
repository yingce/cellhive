...
- **Reserved env namespace**: `CH_*`/`CELL_*`/`__cellhive*` and the platform identity keys (`PLATFORM`/`LOG_NS`/`LOG_WORKER`/`LOG_TOKEN`/`WF_*`) are reserved; user vars/secrets/binding names in those namespaces are rejected at deploy/write time (`reserved_env_name`) so they can never silently collide with the platform control channels.
