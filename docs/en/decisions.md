...
| ADR-186 | Stock-workerd Runtime Baseline and Security Hardening | Pins stock workerd `1.20260916.1` and esbuild `0.28.2`; uses `fromEnvironment`, removes platform credentials from final WorkerCode, generates compatibility rules from pinned source, and enforces 64 MiB code / 1016 KiB env budgets. Code paths and real-workerd suites are implemented; the Docker/no-skip gate remains pending. |
