...
| 项 | 状态 | 证据 / 若失败的回退 |
|---|---|---|
| workerd actor SQLite 是否 WAL 模式、可被外部只读打开 | ✅ 已验证 | P0.7：`walscan` 解析真实 workerd actor `-wal`（workerd 2026-06-15） |
...
| **稀疏文件按需 paging**（冷启动不整库下载） | ✅ **原语已交付**（ADR-055/056；`internal/replica TestColdHydrateIsWholeObjectNotPaged` 实测 cell-agent hydrate 是整对象下载 + 0 ranged 读）：`restore.IndexChain`/`SparseFile`、`replica.PageFetcher` + `L1/index.bin` ranged-get、`restoreverify -bucket`；**GC ✅**（ADR-056）。**消费方边界**：逐页/部分页目前只用于 do-supervisor（DO 文件）与 `restoreverify`；**cell-agent 的 KV/D1/Queue/Workflow/Vectorize cell 冷恢复仍是整库 hydrate**（`replica.Restore` 整读 L1 对象 + L0 尾巴 → `restore.ApplyFile`），稳态读本地。注：SQLite **运行期** VFS 懒读属后续"运行期集成"（非本复制项） |
...
| 租户日志的留存（ADR-136/172） | ⚠️ **边界**：内存 ring 非持久、单节点、忙时丢最旧；OTLP logs 导出默认关（`off`），`tail` 模式只在 `cellhive tail` 的 TTL 订阅期间导出；订阅会经 lease 名单**广播到其它活节点**（ADR-173，best-effort + 每 worker ≤25s 节流，失联节点下次续订补上）；导出 best-effort（队列满/后端不可用丢弃）；日志行无 `request_id` |
...
| R2 `writeHttpMetadata`（ADR-174→ADR-175 已修） | ✅ **已解决**：`loader.js`/`buildFacetEnv` 传 `CH_R2_BINDINGS`，`queue-wrapper`/`bindings-wrapper` 对每个 R2 binding 套本地 Proxy（`facades.js wrapR2Metadata`），`writeHttpMetadata` 在租户 isolate 改调用方 `Headers`，content-type 正常；仅 `get()` 被代理，其余方法透传 |
...
| DO stub 上的 WebSocket 升级（ADR-174→ADR-175 已修） | ✅ **已解决**：facade 检测 `Upgrade` → cell-agent `GET /v1/do/connect`（scoped）取 `{owner,ticket}` → owner do-runtime `/v1/do/connect`（ticket 仅本 shard）；`wsecho` 示例 WS echo 通过。alarm 身份（`storage_id/storage_class`）与 do-runtime 地址 scheme 同批修复 |
...
**含义（2026-09-15 更新）**：P3.7/P3.8 的交付（facet 发现、对象键、按对象恢复、门、跨节点冷激活/接管、compat 套件）**均已实现并验证**。P3 剩余见 roadmap「P3 未完成（遗留）」：**WS 跨节点转发**、**`transferred_classes`**、**SQLite 运行期 VFS 懒读**、`refreshFacets` walk 优化、C 类环境验证。
