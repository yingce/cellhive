...
| 租户 console 日志（ADR-185） | ⚠️ **暂不支持**：实测 pinned stock `workerd 1.20260615.1` 的动态 `workerLoader` 在 `tails: [{name:"tenant-tail"}]` 上拒绝 service designator：`TypeError: Incorrect type for array element 0: the provided value is not of type 'Fetcher'.`。因此已删除 `log-tail.js`/`PlatformBridge.logSend` 旧桥接；tenant 请求与 DO facet 仍正常执行，但 `console.*` 不写入平台 ring 或 OTLP。不得以 tenant env 的 token、URL 或 transport 恢复此路径；升级 pin 后必须重跑真实动态-tail spike。 |
| workerd 运行时基线（ADR-186） | 🚧 **实施中**：目标 pin `1.20260916.1`、esbuild `0.28.2`、宿主 `fromEnvironment`、最终 WorkerCode 64 MiB 与 env 1016 KiB 预算；真实二进制、镜像和 Compose 验收通过前，现行代码 pin/边界仍为运行事实。Tail 恢复不属于本阶段，不得提前标记支持。 |
