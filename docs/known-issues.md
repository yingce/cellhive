...
| 租户 console 日志（ADR-185） | ⚠️ **暂不支持**：pinned stock `workerd 2026-06-15`（npm pin `1.20260615.1`）的动态 `workerLoader` 在 `tails: [{name:"tenant-tail"}]` 上拒绝 service designator（`TypeError: Incorrect type for array element 0: the provided value is not of type 'Fetcher'.`）。因此已删除 `log-tail.js`/`PlatformBridge.logSend` 旧桥接；tenant 请求与 DO facet 仍正常执行，但 `console.*` 不写入平台 ring 或 OTLP。不得以 tenant env 的 token、URL 或 transport 恢复此路径；见 task-3-report，待未来 pinned 版本重新 spike。 |
