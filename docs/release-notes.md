...
- **保留 env 命名空间**：`CH_*`/`CELL_*`/`__cellhive*` 与平台身份键（`PLATFORM`/`LOG_NS`/`LOG_WORKER`/`LOG_TOKEN`/`WF_*`）为保留名，用户的 var/secret/binding 名落在其中会被部署/写入拒绝（`reserved_env_name`），避免与平台控制通道静默冲突。
