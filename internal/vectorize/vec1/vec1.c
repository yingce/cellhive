...
static int vec1ConfigQuantizer(
  ...
  const char *azQuant[] = {"none", "pq", "opq", "bq"};
...
static int vec1Ann1TrainCfg(
  ...
  if( rc==SQLITE_OK ){
    switch( eOpt ){
      ...
      default: assert( sqlite3_stricmp("nopq_round", zOpt)==0 );
        assert( eOpt==10 );  /* nopq_round */
        rc = vec1ConfigInt(eType, iVal, 
...
static int vec1SetupKANNQuery(
  ...
  p = (Vec1Query*)vec1MallocZero(
      ...
      + nElem * sizeof_f32                         /* Vec1Query.aVector[] */
      + bTransform * (nElem * sizeof_f32)          /* aTransform[] */
      + nBit                                       /* aBit[] */
  );
  ...
  if( bTransform ){
    p->aTransform = &p->aVector[nElem];
  }else{
    p->aTransform = p->aVector;
  }
  if( nBit ){
    p->aBit = (u8*)&p->aTransform[ nElem ];
  }
  ...
  if( nVec!=(nElem*sizeof_f32) ){
    ...
    memcpy(p->aVector, aBlob, nElem * sizeof_f32);
  }
